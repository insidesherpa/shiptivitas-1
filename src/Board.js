...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    ...
    const drake = dragula(
      [backlog.current, inProgress.current, complete.current],
      {
        isContainer: el => {
    ...
    drake.on("drop", (el, target, source, sibling) => {
      let swimlaneName = target.parentElement
        .querySelector(".Swimlane-title")
        .textContent.toLowerCase();

      if (swimlaneName === "in progress") {
        swimlaneName = "inProgress";
      }

      this.setState(prevState => {
        console.log(prevState);
        const clients = { ...prevState.clients };
        clients[swimlaneName] = [...target.children].map(element => {
          const id = element.getAttribute("data-id");
          return prevState.clients[swimlaneName].find(
            client => client.id === id
          );
        });
        return { clients };
