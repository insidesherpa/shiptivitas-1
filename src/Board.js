...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    ...
    drake.on("drop", (el, target, source) => {
      // Get the status of the swimlane where the card was dropped
      const newStatus = getNewStatus(target.getAttribute("data-status"));

      // Get the card ID from the data-id attribute of the dragged card
      const cardId = el.getAttribute("data-id");

      // Update the state with the new status for the dragged card
      this.setState((prevState) => {
        // Find the swimlane that contains the dragged card and update its status
        const swimlaneToUpdate = Object.keys(prevState.clients).find(
          (swimlane) =>
            prevState.clients[swimlane].find((client) => client.id === cardId)
        );

        const updatedSwimlane = prevState.clients[swimlaneToUpdate].map(
          (client) =>
        ...
        const updatedClients = {
          ...prevState.clients,
          [swimlaneToUpdate]: updatedSwimlane,
        };
    ...
    const getNewStatus = (data_status) => {
      if (data_status === "backlog") return "backlog";
      else if (data_status === "in progress") return "in-progress";
      else if (data_status === "complete") return "complete";
    };
  }
